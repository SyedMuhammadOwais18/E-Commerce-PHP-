 ...
 $ViewQuery = "SELECT * FROM storage_device inner join product on storage_device.sd_id = product.sd_id";
  ...
  while($DataRow = mysqli_fetch_assoc($Execute)){
   // $m_id = $DataRow['mobile_id'];
     ...
     $Images[] =  array('s_id' => $DataRow['sd_id'],'s_name'=> $DataRow['storage_name'],'s_img1'=> $DataRow['img1'],'s_stock'=> $DataRow['stock_quantity'],'s_price'=> $DataRow['sd_price']);
    ...
    <li class="nav-item" >
      ...
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu" aria-labelledby="navbarDropdown">
          ...
          <a class="dropdown-item active" style="color: white; font-weight:bold" href="storagedevice_cat.php">Storage Device</a>
        </div>
      </li>
      <?php
...
<div class = 'container'>
    <div class = 'row'>
        <div class = 'col-sm offset-sm-4'>
            <h1 style = 'margin-bottom:40px; margin-top:20px;'> Storage Device</h1>
...
<div class = 'row'>
  ...
  <?php 
    ...
    <div class = 'col-sm-3 offset-sm-1'>
            ...
            <div class = 'card'>
                <div class = 'card-body'>
                    <img src = '<?php echo $im['s_img1']; ?> ' alt = 'img1' class = 'img-responsive' height = '150' style = 'width : 150px;  margin-left:auto; margin-right:auto; display:block;'>
        
                    <div style = 'text-align:center;'>
                        <a href = '#'> <?php echo $im['s_name']; ?></a>
                    </div><!-- for name-->
                    <div style = 'text-align:center;'>
                     Price : <?php echo $im['s_price']; ?>
                    </div>
                    <div style = 'text-align:center;'>
                     availability : <?php if($im['s_stock']>0){
                         echo "in stock";
                     }else{
                         echo "out stock";
                        } ?>
                ...
                <form action = 'storagedevice_cat.php' method = 'post'>
                            <input type = 'hidden' name = 'price' value = <?php echo $im['s_price'] ; ?>>
                            <input type = 'hidden' name = 'stock' value = <?php echo $im['s_stock'] ; ?>>
                            <input type = 'hidden' name = 's_id' value = <?php echo $im['s_id'] ; ?>>
                        ...
                        <button class = 'btn btn-danger flex-fill' ><a href = 'storagedevice_detail.php?sd_id=<?php echo $im['s_id'];  ?>' style = 'color:white'>More Details</a></button>
                        </form>
                        <!-- <button class = 'btn btn-danger flex-fill' ><a href = 'storagedevice_detail.php?sd_id=<?php echo $im['s_id'];  ?>' style = 'color:white'>More Details</a></button> -->
 ...
 if(isset($_POST['submit'])){
  ...
  function check_stock()
  {
    ...
    if($check_stock<0)
    {
        ...
        echo "<script>windows.location.href = 'storagedevice_cat.php'</script>";
        return 0;
    }
    else
    {
   ...
   // print_r($_SESSION);
        ...
        //      echo "<script>windows.location = 'mobile_inf.php'</script>";
            ...
            //  echo $count;
            if(check_stock()){
             $item_array = array('id' => $_POST['s_id'],'quantity' => $_POST['qty'],'stock' => $_POST['stock']);
             //print_r($item_array);
            ...
            {
            echo "<a href = 'storagedevice_cat.php'></a>";
            }
        //      print_r($_SESSION);
            }
    //     //print_r($_SESSION);
